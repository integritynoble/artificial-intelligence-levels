...
\hypersetup{
  pdftitle={Unified Intelligence Theory and the Artificial Intelligence Level: A Cumulative, Harness-Measurable Hierarchy},
  pdfauthor={\pdfauthorlist},
...
%% Float placement, retuned for the narrower TMLR text block. LaTeX's
%% defaults leave this paper (46 tables) with table-only pages and wide
%% gaps; these are the standard parameters, and they move no content.
...
%% Keep floats inside the section that introduces them. Without this,
%% tables drift above their own heading and a section reads as empty.
\usepackage[section]{placeins}
%% placeins only guards \section; this paper's tables belong to
%% subsections, so add the same barrier there.
\let\dkoldsubsection\subsection
\renewcommand{\subsection}{\FloatBarrier\dkoldsubsection}
\let\dkoldsubsubsection\subsubsection
\renewcommand{\subsubsection}{\FloatBarrier\dkoldsubsubsection}
...
\title{Unified Intelligence Theory and the\\
...
%% Author record from DK-02's author_record.json; the anonymous
%% build hides this block. Order and consent are the authors' to confirm.
\author{
      ...
      \addr NextGen PlatformAI C Corp, USA
      \AND
      \name Dingyi Kang \email dingyi.kang@utdallas.edu \\
      \addr University of Texas at Dallas, USA}
...
\item A separation of five conceptual intelligence families into six directly measured coordinates,
      $[C,I,O,T,H,SA]$, with Delegation Intelligence decomposed rather than treated as a primitive scalar, and GUI/screen cognition placed inside $C$ as a domain with a diagnostic perceptual subscale rather than as a family or a ladder (\S3--\S7, \S\ref{sec:gp}).
...
\item A continuous pair score HLIS with a fully specified construction, and a harness-indexed characterization AI-Level of a base model across a standardized ladder (\S13--\S14).
...
\item One testing method for every ladder: the six-part schema in which $I$ was already written --- construct as contrast, separate prerequisite, witness with a control arm, factor gate times retention, certification law, continuous relaxation --- instantiated for $C$, $SA$, $O$, the $(T,H)$ surface, $HG$ and $U$, and enforced as data in the benchmark (\S\ref{sec:onestyle}).
...
\subsection{GUI-Grounded Cognition: the \texorpdfstring{$C^{\mathrm{GUI}}$}{C-GUI} Domain and the GP Diagnostic Subscale}\label{sec:gp}

Computer-screen understanding is a first-class capability for practical agents and is not a new top-level family, nor a replacement ladder. The framework places it \emph{inside} Cognitive Intelligence in two distinct roles. The $C$ ladder continues to answer how deep the cognition is --- reactive, contextual, compositional, strategic, expert, discovery, frontier-generalising, open-ended; a GUI capability specifies \emph{where} that cognition is grounded. So GUI/screen is a domain of $C$, with GUI-grounded witnesses
\[
C^{\mathrm{GUI}}_0 \to C^{\mathrm{GUI}}_1 \to \cdots \to C^{\mathrm{GUI}}_6 \to C^{\mathrm{GUI}}_{\Omega},
\]
where $C^{\mathrm{GUI}}_k$ is a GUI-domain witness of the existing semantic level $C_k$ and not a new level, and GUI Perception $GP0$--$GP5$ is a diagnostic subscale of the representation supplied to cognition. The containment shorthand and its essential non-equivalence are
\[
GP \subset C^{\mathrm{GUI}} \subset C, \qquad GP_g \not\equiv C_g .
\]
...
\[
D_C = \{\text{general}, \text{science}, \text{mathematics}, \text{abstract}, \text{code}, \text{multimodal/spatial}, \text{GUI/screen}, \text{long-context}\},
\]
and a score in one domain never silently substitutes for another. Until ratification calibrates mandatory domain floors, a system without image or screen access reports $C^{\mathrm{GUI}} = \text{N/A}$ rather than zero, and a broad multimodal $C$ claim must state whether GUI/screen is required.
...
\midrule
$C^{\mathrm{GUI}}_0$ & Reactive & Read, identify or localise one explicitly requested screen element with no hidden planning requirement. \\
$C^{\mathrm{GUI}}_1$ & Contextual & Infer the ordinary function of a visible interface element from screen context and a routine goal. \\
$C^{\mathrm{GUI}}_2$ & Compositional & Combine several GUI elements, states, spatial relations, panels, rows or constraints to answer a structured question. \\
$C^{\mathrm{GUI}}_3$ & Strategic & Construct or select an interface-navigation strategy and transfer it to a held-out layout or unfamiliar application surface. \\
$C^{\mathrm{GUI}}_4$ & Expert & Reason robustly through a dense expert interface containing ambiguity, interacting constraints and relevant distractors. \\
$C^{\mathrm{GUI}}_5$ & Discovery & Infer an unknown interface rule, workflow, transition model or interaction mechanism and predict sealed GUI states. \\
$C^{\mathrm{GUI}}_6$ & Frontier-generalising & Integrate GUI understanding with several genuinely necessary domains: code, mathematics, science, documents, spatial reasoning. \\
$C^{\mathrm{GUI}}_{\Omega}$ & Open-ended & Create a new interface representation, parser, abstraction or method that causally expands the classes of GUI-grounded problems subsequently reachable. \\
\bottomrule
\end{tabularx}
\caption{GUI-grounded witnesses of the existing $C$ ladder. Each row is a domain instance of a level that already exists; the gate is the ordinary cumulative one, $z_{C^{\mathrm{GUI}},k} = V_{\mathrm{GUI},C_k}\cdot K_{C,<k}$, and no $GP$ evidence enters it.}
\label{tab:cgui}
\end{table}

GUI perception is broader than OCR. A screen-reading system must recognise text and icons, identify interface objects, ground them spatially, interpret interface state, track change across frames, and generalise to layouts it has never seen. $GP$ measures the fidelity and generality of that representation and must not be mapped one-to-one onto $C$ levels: no rule of the form $GP3 \equiv C3$ is permitted, although a form may declare a minimum $GP$ prerequisite for native-image variants.
...
\bottomrule
\end{tabularx}
\caption{GUI Perception subscale GP0--GP5, a diagnostic of $C^{\mathrm{GUI}}$ that reports beside $C$ and never promotes it.}
\label{tab:gp}
\end{table}

\paragraph{Failure decomposition and construct boundaries.} A computer-use failure can arise from perception, reasoning, action execution, or delegation-process control, and the measurement keeps the four legible rather than collapsing them into one score:
\[
\text{computer task outcome} \Leftarrow GP \;\wedge\; C^{\mathrm{GUI}} \;\wedge\; \text{action fidelity} \;\wedge\; DI \text{ process}.
\]
$GP$ measures screen perception; $C^{\mathrm{GUI}}$ measures $C$-level reasoning grounded in a GUI; action fidelity measures whether requested clicks, typing, scrolling and dragging are executed correctly and is not itself a $C$ level; $DI$ measures whether the complete task can be delegated at $(T, H, p)$. The controls that separate them are matched arms. A \emph{native-screenshot arm} gives the candidate only the declared screen stream and permitted non-oracle tools. An \emph{oracle-screen arm} gives it the exact hidden screen graph $G^{*}_{\mathrm{GUI}}$ with task, budget and verifier fixed, and defines
\[
\Delta_{GP} = Q(A \mid G^{*}_{\mathrm{GUI}}) - Q(A \mid I_{\mathrm{screen}});
\]
a large positive $\Delta_{GP}$ says native performance is perception-limited, and oracle-arm success is diagnostic only --- it never replaces native $C^{\mathrm{GUI}}$ evidence when native grounding is part of the claim. A \emph{perfect-actuator arm} executes requested high-level actions perfectly while preserving the same screen transitions, and defines $\Delta_{\mathrm{act}} = Q(A \mid \text{actuator}^{*}) - Q(A \mid \text{actuator}_{\mathrm{native}})$, separating cognitive failure from pointing, typing and tool-control error. An optional \emph{oracle-strategy arm} supplies a correct high-level strategy without target coordinates. The minimum failure-attribution record is
\[
F_{\mathrm{GUI}} = \big(q_{\mathrm{native}},\, q_{\mathrm{oracle\text{-}screen}},\, q_{\mathrm{perfect\text{-}actuator}},\, q_{\mathrm{oracle\text{-}strategy}},\, \Delta_{GP},\, \Delta_{\mathrm{act}}\big),
\]
and the same delivered-success event is never counted twice as both $C$ and $DI$ evidence.

\paragraph{Difficulty within a level.} Headroom in the GUI domain comes from a difficulty vector that varies \emph{inside} a $C$ level and never creates a pseudo-level: visible element count $N_e$, panels and windows $N_p$, hierarchy depth $d_h$, distractor density $r_d$, occlusion $\rho_o$, windows and tabs $N_w$, relevant interface states $N_s$, transition depth $d_t$, layout novelty $\nu_l$, and the resolution or visual budget $B_{\mathrm{res}}$. This is the same principle as the stress vector of \S\ref{sec:validity}: a saturated witness is a historical anchor, and discrimination moves to a harder construct-equivalent form of the same level.

\paragraph{Reporting.} A GUI-capable system reports at least $C \text{ profile} \mid C^{\mathrm{GUI}} \text{ level} \mid GP \text{ level} \mid \text{GUI task } T/H/p$; a system may be globally $C4$ with $C^{\mathrm{GUI}}_3$, $GP5$ and a computer-use delegation frontier of $T3/H1@p{=}0.80$, and those answer different questions. A computer-agent episode decomposes into $[P, G, S, A, R]$ --- perception, grounding, interface-state understanding, action selection and result recognition --- of which $P$, $G$, $S$ diagnose $C$ through $GP$ and $A$ with the closed loop are evidence for $DI$ and action fidelity.
...
\section{One Testing Method for Every Ladder}\label{sec:onestyle}

The Individual ladder is written in a particular style, and it is the style that makes its levels invariant rather than the particular capabilities it names. Each $I_n$ is (i) a construct stated as a contrast that must hold; (ii) a prerequisite that is checked separately and can never promote; (iii) a witness with a control arm that the construct must beat --- the no-record arm at I1, the no-experience arm at I2, the ablated $\Theta$ at I3, the frozen $\Psi$ at I4, the matched control after restart at I5, the pre-frontier at I\Om{}; (iv) a gate that is a product of named binary factors, each computed by a locus outside the pair's write set, times retention of every lower level; (v) a certification law that says a level is held iff the gate is one at the ratified reliability; and (vi) a continuous relaxation of the same product for the pair score. Nothing in that list is specific to Individual Intelligence, and a framework in which one ladder is tested this way and the others by description has two standards. This section states the schema once and instantiates it for every ladder, sub-ladder and gate in the framework: $C$, $SA$, $O$, the delegation surface $(T, H)$, the harness generations $HG$, and $U$ itself. $I$ and $M$ already have the form (\S\ref{sec:memory}, \S\ref{sec:mops}).

\begin{claimbox}{The schema}
For a ladder $X$ and a level $n$, with $A$ the tested pair (or, for $HG$, the tested harness):
\begin{enumerate}[leftmargin=*,itemsep=1pt]
\item \textbf{Construct} $\Phi_{X,n}$: a contrast that must hold, or a structural property of the item; never a difficulty threshold.
\item \textbf{Prerequisite} $\pi_{X,n}$: what must already be certified --- lower-level retention $K_{X,<n} = \prod_{j<n} K_{X,j}$ always, plus any cross-ladder prerequisite ($M_{\mu_n}$ for $I_n$; every coordinate for $U_n$; the previous rung's mechanism for $HG_g$). A prerequisite can block and can never promote.
\item \textbf{Witness} $W_{X,n}$ with a \textbf{control arm} $\bar W_{X,n}$: the canonical episode, and the arm the construct must separate from --- ablated, withheld, twin, previous rung, single role, or oracle. A witness without a control arm tests an assertion, not a contrast.
\item \textbf{Factor gate} $z_{X,n}(A) = \prod_{f \in F_{X,n}} f(A) \cdot K_{X,<n}(A)$, every factor binary and computed by a locus the pair cannot write to.
\item \textbf{Certification law} $A \models X_n \iff \pi_{X,n}(A) \wedge z_{X,n}(A) = 1$ at reliability $p$ over the ratified number of episodes; $p$, the episode count and any margin are ratified constants, not laws.
\item \textbf{Continuous achievement} $q_{X,n} \in [0,1]$, the mean over episodes of the same product with each factor relaxed to its score, cumulated as $q^{*}_{X,n} = \min_{j \le n} q_{X,j}$ for HLIS (\S\ref{sec:hlis}).
\end{enumerate}
Two consequences follow. No level can be skipped in any capability ladder, because $K_{X,<n}$ is a factor --- and which objects are capability ladders is itself typed (\S\ref{sec:typed}). And no ladder is certified by a description, because a factor that no locus computes is not a factor.
\end{claimbox}

\subsection{Cognitive Intelligence}
The witness at every $C_k$ is a generated instance carrying the level's trap; the control arm is the trap itself --- the named wrong method a competent system takes, which admissibility proves fails on every seed. The common factors are $S$ (the sealed key is matched) and $\mathcal A$ (admissibility: computed key, trap fires, unique identifiability). The same gate is instantiated once per domain of $D_C$, and $C^{\mathrm{GUI}}$ adds the oracle-screen and perfect-actuator arms of \S\ref{sec:gp}.
\begin{table}[!htbp]
\centering\small
...
C\Om{} & $R_{\mathrm{reach}}$: the post-frontier strictly contains the pre-frontier on disjoint tests & $R_{\mathrm{reach}}\cdot\mathcal A\cdot K$ \\
\bottomrule
\end{tabularx}
\caption{The $C$ ladder in the schema. $K = K_{C,<k}$ throughout. C0--C5 are implemented as generators with a trap that the self-test proves fires; C6 and C\Om{} are specified.}
\label{tab:cstyle}
\end{table}

\subsection{Operational Self-Awareness}
\begin{table}[!htbp]
\centering\small
...
SA4 & $P$: gains, regressions and affected capabilities predicted before a $\Theta$ change is evaluated; $M$: measured after; $A$: agreement above margin & the post-hoc constant forecast at the pair's base rate & $P\cdot M\cdot A\cdot K$ \\
...
SA\Om{} & $L$: a provenance-linked self-model survives continued cognitive or objective evolution & the pre-evolution self-model & $L\cdot K$ \\
\bottomrule
\end{tabularx}
\caption{The $SA$ ladder in the schema. SA-cal, the blind forecast scored by Brier, is a diagnostic beside the ladder and not a rung. SA1--SA4 are implemented; SA5--SA\Om{} are specified.}
\label{tab:sastyle}
\end{table}

\subsection{Organizational Intelligence}
The $O$ ladder mirrors $I$ level for level, with one factor that $I$ never needs: $N_O$, organizational necessity --- the single-role arm, one prompt role-playing several agents, must fail. An organization is a structural property of the witness (real state, authority and evidence boundaries), checked by admissibility, and $N_O$ is what stops parallel individual work from certifying collective intelligence.
\begin{table}[!htbp]
\centering\small
\begin{tabularx}{\textwidth}{@{}lYYl@{}}
\toprule
...
O3 & $\Theta_O$ declared; $M$: modified, in scope, activated; $V$: sealed-suite gain at margin; $G$: proposer, evaluator and promoter separated & the ablated $\Theta_O$ & $M\cdot V\cdot G\cdot N_O\cdot K$ \\
O4 & $\Psi_O$ improves: the mechanism that discovers organizational improvements is itself changed and the change is used on hidden meta-probes & the frozen $\Psi_O$ & $M_{\Psi}\cdot V_{\Psi}\cdot G_{\Psi}\cdot N_O\cdot K$ \\
...
O\Om{} & $R_{\mathrm{reach}}$: new agent types, roles, tools or organizations expand the collective frontier on disjoint tests & the pre-frontier organization & $R_{\mathrm{reach}}\cdot N_O\cdot K$ \\
\bottomrule
\end{tabularx}
\caption{The $O$ ladder in the schema. O0--O2 are implemented (the Core's coordination, log and adaptation families); O3--O\Om{} are specified as mirrors of I3--I\Om{}.}
\label{tab:ostyle}
\end{table}

\subsection{Delegation: the \texorpdfstring{$(T, H)$}{(T, H)} surface}
Delegation has no ladder of the pair's own; it has a surface, and the ``level'' is the frontier $DF_{h,p}(A) = \max\{T : S_{\mathrm{net}}(A, T, H \le h) \ge p\}$. The schema applies to each cell $(b, h)$ of the surface. A band $T_b$ is a structural property of the item and is checked by admissibility, not by pass rates: T1 a small procedure must be inferred; T2 dependent steps with a verifier; T3 strategy choice, uncertainty or recovery required; T4 several planning cycles and a verification burden; T5 the method initially unknown; T6 a mission that must be turned into projects; T\Om{} a charter that must be turned into missions. A class $H_h$ is a property of the transcript: every human intervention is classified by the cognition it supplied --- H0 none; H1 an unavailable fact or narrow clarification; H2 bounded clarification or local correction; H3 periodic review; H4 repeated guidance; H5 most major steps --- by a locus outside the pair.
\[
z_{T_b \mid h}(A) = \Delta \cdot V \cdot \neg FC \cdot L_h \cdot B, \qquad z_{DF}(A) = S_{\mathrm{net}} \cdot K_{T,<b \mid h},
\]
where $\Delta$ is delivery, $V$ the verifier's pass, $\neg FC$ that no false completion was charged (at $\rho$), $L_h$ that the intervention ledger contains nothing above class $h$, and $B$ that the band audit passes. The $T$ cell carries no retention factor, because $T$ is an axis of the item and not an ability; the retention $K_{T,<b\mid h}$ --- every lower band meets the same reliability at the same class --- belongs to the frontier cell (\S\ref{sec:typed}). The witness is the four-outcome record of \S\ref{sec:adi}; the control arms are two: the \emph{held-back arm}, in which a refusal must score above a false completion (the contrast the success-only primitive could not see), and the \emph{ledger}, in which an intervention that supplied strategy is class H3 or above whatever the pair calls it. $A \models T_b/H_h \iff z_{DI,(b,h)} = 1$ at reliability $p$, and the frontier is the largest $b$ for which that holds at $h$.

\subsection{Harness generations}
A rung is a property of the harness, not of the model, and is certified in the same form. The factors are $E$, the mechanism exists in the frozen harness contract; $U$, the mechanism \emph{fires} on the witness; and $\Delta_g$, the rung's contrast with the previous rung on the same seeds --- the outcome the mechanism is supposed to change does change. The witness is the harness-scaling curve; the control arm is the previous rung with the same model and the same seeds, which is why the curve is measured that way (\S\ref{sec:measured}).
\begin{table}[!htbp]
\centering\small
\begin{tabularx}{\textwidth}{@{}lYl@{}}
\toprule
Rung & $U$ (the mechanism fires) and $\Delta_g$ (what must change against the previous rung) & Gate \\
\midrule
...
HG4--HG6 & the governed $\Theta$ loop, the $\Psi$ engine, the unknown--hypothesis--experiment loop, the organization exist and fire; each enables the matching $I$/$O$/$SA$ campaign and earns none of it & $E\cdot U\cdot\Delta_g\cdot K$ \\
HG\Om{} & validated discovery is transduced into a new instrument, agent or organization that the frontier tests reach & $E\cdot U\cdot\Delta_\Omega\cdot K$ \\
\bottomrule
\end{tabularx}
\caption{The $HG$ ladder in the schema. $K = K_{HG,<g}$: a rung keeps every lower mechanism. HG0--HG3 are built; HG4--HG\Om{} are specified. Architecture enables a claim; only the model's own campaign earns it.}
\label{tab:hgstyle}
\end{table}

\subsection{The Unified level}
...
\[
\begin{aligned}
z_{U,n}(A) &= \prod_{X \in \{C, I, O, SA\}} \mathds{1}[X(A) \ge X_n] \cdot \mathds{1}[S_{\mathrm{net}}(A, T_n, H \le H_n) \ge p_n] \cdot K_{U,<n}(A),\\
A &\models U_n \iff z_{U,n}(A) = 1,
\end{aligned}
\]
with the minima $X_n$ and the delegation gate $(T_n, H_n)$ from Table~\ref{tab:ugates}. The report names the coordinate that fails: \texttt{U2 [C5, I4, O2, T4/H1, SA4] --- bottleneck: O2} is the control arm made visible.

\subsection{Cumulative structure is typed, not universal}\label{sec:typed}
The schema's retention factor $K_{X,<n}$ must not be applied to every ordered object as if all were ladders of the same kind. Four relations hide under the word ``cumulative'', and each object in the framework has exactly one of them.
...
\midrule
$C$, $I$, $M$, $O$ & capability ladder & \textbf{retention}: $A \models X_k \iff V_{\mathrm{new},X,k} \wedge K_{X,<k}$ & a higher level must retain every lower capability --- of capability, not of implementation \\
$SA$ & capability ladder from SA1 & retention from SA1; SA0 is not retained & SA0 denotes the absence of a grounded self-model, and SA1 cannot retain an absence \\
$GP0$--$GP5$ & diagnostic sub-ladder & retention inside $GP$; never promotes $C^{\mathrm{GUI}}$, $C$ or $U$ & objects $\to$ grounding $\to$ state $\to$ dynamics $\to$ novel interface build on one another \\
$U$ & gated integration ladder & retention of every lower $U$ and every coordinate certificate & no continuous score compensates a failed bottleneck \\
$DI$ & delegation frontier & \textbf{frontier}: $DF_{h,p} = \max\{T_b : \forall j \le b,\ S_{\mathrm{net}}(T_j, H \le h) \ge p\}$ & a pass at a hard band cannot leap a failed easier band; lower-$T$ retention lives here and nowhere else \\
$T$ & ordered axis & \textbf{none}: no $T$ retention in a $T$ item certificate & $T$ labels the structure of the task, not an ability of the system \\
$H$ & ordered axis & \textbf{none}: the class is a property of the transcript & $H$ classifies the assistance supplied; lower is less \\
$HG$ & engineering reference ladder & \textbf{nesting}: $HG_g = HG_{g-1} + \Delta_g$ & a strict mechanism superset is what makes harness gain attributable; architecture, not intelligence \\
\bottomrule
\end{tabularx}
\caption{Cumulative structure by object. Floors ($X0$) are structural baselines and are not retained capabilities; a cross-ladder prerequisite ($I_n$ needs $M_{\mu_n}$, $U_n$ needs every coordinate) is a one-way gate and not cumulativity.}
\label{tab:typed}
\end{table}

For a capability ladder $X \in \{C, I, M, O, SA_{1:\Omega}, GP, U\}$ the general law is $K_{X,<k} = \prod_{j<k} K_{X,j}$ and $A \models X_k \iff V_{\mathrm{new},X,k}(A) = 1 \wedge K_{X,<k}(A) = 1$, with any independent cross-ladder prerequisite added conjunctively; the continuous form is $q^{*}_{X,k} = \min_{j \le k} q_{X,j}$. A system that passes a C5 discovery witness and fails C2 composition has evidence of C5-type behaviour and no C5 certificate; a domain-specific $C$, including $C^{\mathrm{GUI}}$, inherits the ordinary $C$ retention gate rather than creating a second ladder. The law is not applied literally to $T$ or $H$: the $T$ cell of \S\ref{sec:onestyle} therefore carries the band audit $B$ and no $K$, and the retention $K_{T,<b\mid h}$ belongs to the frontier. An HG4-conformant harness can carry an I2 system: the rung is cumulative architecture, and the campaign the architecture enables is what earns a level. The measurement contract records the type on every cell and refuses a $T$ or $H$ record that carries retention, a $GP$ record missing a lower $GP$, a frontier that skips a failed band, a $GP$ record that claims promotion, and an $HG$ rung missing a lower mechanism. This changes the contract, not the meaning of any label.

\subsection{What the uniform style buys}
Three things. First, every ladder now fails in the same way --- a named factor is zero --- so a failed certification is a diagnosis, not a score. Second, the levels of every ladder are invariant for the same reason $I$'s are: a contrast does not move when the frontier moves. Third, the implementation can enforce the form: in the benchmark, every law carries its factors, its control arm and its prerequisite as data, and a test refuses a law without them. The entries for ladders this work has not run --- C6, C\Om{}, SA5--SA\Om{}, O3--O\Om{}, HG4--HG\Om{}, the upper $U$ --- are marked specification, which is the same word \S\ref{sec:limitations} uses for them.

\subsection{The harmonized grammar, and the grid}
The measurement standard's harmonized method record --- capability being tested, experimental intervention, hidden or grounded state, ordered procedure, controls and ablations, primary metrics, certification gate, required lower levels, resource envelope, binding status --- is this schema in a different order. The capability is the construct and the intervention is its contrast; the controls entry is the control arm; the certification gate is the factor gate with retention as its last factor; who computes each metric is the factor's locus; the binding status is the cell's status. The two grammars therefore name the same cells, and the benchmark instantiates the schema as a \emph{grid}: one witness for every level of every ladder, each carrying all six parts and a status that is a fact about the package rather than about the level, generated from data so that the paper and the dataset cannot disagree. What this paper adds to that grammar is the requirement that the controls entry be a contrast the construct must beat and that every factor name its locus; a record with a description in the controls field and a pass rate in the gate field is a description, not a test.
...
\section{Overall Scoring for One Harness-LLM Pair}\label{sec:hlis}
...
\subsection{HLIS Is the Building Block of AI-Level}
...
\section{Artificial-Intelligence-Level (AI-Level) Characterization of a Base LLM}
...
\subsection{The Components of the AI-Level Score: Level, AUC, Ceiling and Harness Gain}
...
\midrule
Level$(m)$ & Highest standardized generation $k$ for which $A(m,HG_k)$ satisfies the cumulative target gate. & How far up the ladder the LLM can validly exploit the architecture. \\
AUC$(m)$ & Weighted mean of $HLIS(m,HG_k)$ across tested generations. & Breadth across harness complexities. \emph{See the caution in \S\ref{sec:composite}.} \\
Ceiling$(m)$ & $\max_k HLIS(m,HG_k)$. & Best score a standardized harness realizes from the model. \\
...
\bottomrule
\end{tabularx}
\caption{AI-Level metrics for a base LLM across a standardized harness ladder.}
...
\subsection{A Single AI-Level Score, and Two Cautions}\label{sec:composite}

For applications requiring one 0--100 number:
\begin{equation}
\text{AI-Level Score} = 0.55 \times \text{AUC} + 0.35 \times \text{Ceiling} + 0.10 \times \text{Harnessability}
\end{equation}
where Harnessability is a 0--100 normalization of positive Harness Gain. These weights are a proposal rather than a law. The three components and the rung reached must always be published beside the composite.

\begin{cautionbox}{Two structural cautions}
\textbf{AUC and Ceiling are computed from the same series.} Ceiling is the maximum of the values AUC averages, so their correlation is structural rather than empirical, and $0.55 + 0.35 = 0.90$ of the weight sits on two views of one quantity. The only component not derivable from the others carries $0.10$.

\textbf{AUC does not measure harnessability.} Consider a flat model scoring $55$ at every rung and a steep one scoring $20, 35, 55, 75, 92$. Their AUCs are $55.0$ and $55.4$ --- essentially equal --- though only one converts architectural support into verified intelligence, which is the property \S14.1 says AI-Level exists to measure. The composite does separate them, through the two terms carrying least weight.

Until calibration settles this, \textbf{the curve itself is the primary result} and the composite is a convenience. Report $\langle$Level, $HLIS(m,HG0)$, Ceiling, Harness Gain, ladder id$\rangle$ --- the ordinal statement plus the curve's start, top and rise.
\end{cautionbox}
...
Level + curve & One base LLM across the ladder. & How much intelligence the model expresses when harnessed. \\
...
\section{Benchmark Library for the AI-Level}
...
AI-Level does not replace these. Let $B(m)$ denote the vector of contemporary results. The AI-Level program \textbf{consumes} $B(m)$ as its evidence layer --- $A_C$ is built from it --- and then asks a different question: how does the same frozen model behave when embedded in progressively stronger standardized harnesses?
...
\bottomrule
\end{tabularx}
\caption{Mapping contemporary benchmarks into the AI-Level architecture.}
\label{tab:mapping}
\end{table}
...
Ceiling & \textbf{96.7} & \textbf{96.7} & 93.3 & and still ends behind \\
Harness Gain & 3.3 & 3.3 & \textbf{15.0} & but is \textbf{4.5$\times$ more harnessable} \\
AUC & \textbf{95.0} & \textbf{95.0} & 88.8 & AUC ranks by baseline, not by harness response \\
\bottomrule
\end{tabularx}
...
Harness Gain does what \S14.2 claims: it distinguishes an executor that converts architectural support into verified intelligence from one already near its ceiling. The frontier executors had 6.7 points of headroom and used 3.3; the weaker had 21.7 and used 15.0. \textbf{AUC ranks them in the opposite order}, which is the caution of \S\ref{sec:composite} with data behind it, and the reason the curve is primary here and the composite a convenience.
...
\subsection{Why AI-Level Adds Information Beyond a Benchmark Average}
...
\bottomrule
\end{tabularx}
\caption{What a typical benchmark score reports versus the AI-Level extension.}
\label{tab:hilext}
\end{table}
...
AI-Level layer & $A_C$ profile; \SA{} and \FA{}; false-completion rate; I/O/SA levels; $U^{*}$; per-harness HLIS; the curve; Level; Ceiling; Gain; \rhov{}; optional composite. & How the model becomes a persistent, delegated, organizational, self-modeling system as harness sophistication increases. \\
\bottomrule
\end{tabularx}
...
\textbf{Four rungs of eight.} HG4--HG\Om{} are not built, so Level above 3 is not assessable and AUC is a mean over a truncated ladder --- which makes the measured composite incomparable with one computed over a full ladder.
...
Unified Intelligence U0--U\Om{} is a cumulative gated hierarchy: promotion requires retention of all lower-level capabilities and satisfaction of minimum requirements in every coordinate. HLIS scores one frozen pair; AI-Level characterizes a base model across a standardized ladder; and the Harness Scaling Curve is the primary result from which the summaries are drawn.
...
\section{AI-Level Reporting Template}
...
\item \textbf{LLM summary:} benchmark vector $B(m)$ | Harness Scaling Curve | Level | Ceiling | Harness Gain | \rhov{} | AUC and composite, if reported, with the cautions of \S\ref{sec:composite} | HDS if harness-design tasks were run.
