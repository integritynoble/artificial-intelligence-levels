...
|---|---|
| ~~Matching editable v2.6 source was not found in the inspected bundle.~~ Resolved 2026-09-17: source, bibliography and figures are published in [`paper-sources/`](paper-sources/README.md). | Build that source and compare every section, equation, table and figure against the preserved PDF; the [source handover](PAPER_SOURCE_HANDOVER.md) records the known differences (the source is ahead of v2.6 and folds in the appended addenda). Do not treat an older draft as the matching source. |
